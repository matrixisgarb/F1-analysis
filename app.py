...
# Add driver labels with vertical offset to avoid bunching
...
# Sort by final lap time and space labels out
...
# Highlighted driver by compound
compound_colours = {
    'SOFT': '#e8002d',
    'MEDIUM': '#ffd700',
    'HARD': '#ffffff',
...
# Pit markers — clamped inside chart
...
for driver in laps['Driver'].unique():
    ...
    ax.text(last['LapNumber'] + 0.3, last['LapTimeSeconds'],
            driver, fontsize=6, color='#888899', va='center')
...
for _, pit_lap in pit_laps.iterrows():
    ax.axvline(x=pit_lap['LapNumber'], color='#ff8000',
               linestyle='--', alpha=0.4, linewidth=1)
    ax.text(pit_lap['LapNumber'] + 0.3, ax.get_ylim()[0] + 0.5,
            'PIT', fontsize=7, color='#ff8000', alpha=0.7)
...
if compare_driver != 'None' and compare_driver != selected_driver:
    ...
    for _, pit_lap in compare_pit_laps.iterrows():
        ax.axvline(x=pit_lap['LapNumber'], color='#00bfff',
                   linestyle='--', alpha=0.4, linewidth=1)
        ax.text(pit_lap['LapNumber'] + 0.3, ax.get_ylim()[0] + 0.5,
                'PIT', fontsize=7, color='#00bfff', alpha=0.7)
...
>>>>>>> 50722b2d0f0cd328c14f85ad291ac9bf37a270a0
